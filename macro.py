...
def get_lme_spread():
    ...
    try:
        r = shared_session.get(url, timeout=10)
        if r.status_code == 200 and '3-months' in r.text:
...
def extract_factors(api_key):
    ...
    if walcl and rrp and tga:
        ...
        if min_len > 30:
            ...
            # 简化版平滑
            smoothed = [sum(raw_deltas[i:i+5])/5 for i in range(len(raw_deltas)-5)]
...
def fetch_macro_indicators(fred_api_key=None):
    ...
    # 模块三：经典九大核心观测面板 (你最想要看的数据原值)
    md += "## 📈 核心宏观全景面板 (经典9大指标与因子Z值)\n\n"
    md += "| 变量名称 | 最新绝对数值 | 因子状态 (Z-Score) | 核心驱动逻辑 |\n"
    md += "| :--- | :--- | :--- | :--- |\n"
    
    md += f"| **A. LME铜升贴水** | **{raw['lme_spread']}** | 物理逼空 | 现货贵于期货(>0)说明实体库存极缺。 |\n"
    md += f"| **B. 逆回购(RRP)** | **{raw['rrp']:.1f} B** | **{raw_f['liq_delta_z']:.2f}** (净流动性) | 水库释放器。连同TGA构成核心美元水位。 |\n"
    md += f"| **C. CNH-CNY 价差** | **{raw['cnh_cny']}** | 离岸情绪 | 价差扩大意味着外资在离岸市场大肆做空人民币。 |\n"
    md += f"| **D. 盈亏通胀率** | **{raw['t10']}%** | 远期定价 | 黄金的助推剂，突破前高需重仓。 |\n"
    md += f"| **E. 实际利率(TIPS)**| **{raw['rr']:.2f}%** | **{raw_f['z_realrate']:.2f}** | 黄金绝对反向锚。极高位时压制一切估值。 |\n"
    md += f"| **F. 高收益债利差** | **{raw['hy']:.2f}%** | **{raw_f['z_hy']:.2f}** (信用) | 企业违约生死线，破 5% 亮红灯。 |\n"
    md += f"| **G. 长短端利差** | **{raw['yc']:.2f}%** | 周期警报 | 10Y-2Y 倒挂解除瞬间，通常衰退正式兑现。 |\n"
    md += f"| **H. 美元指数(DXY)** | **{raw['dxy']:.2f}** | **{raw_f['z_dxy']:.2f}** | 全球流动性虹吸指标，走强利空大宗与新兴市场。 |\n"
    md += f"| **I. 复合风险指数** | 综合测算 | **{risk_comp:.2f}** | 结合 VIX, MOVE, HY。>1.5为高危区严禁逆势。 |\n"
